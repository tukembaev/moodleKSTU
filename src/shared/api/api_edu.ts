...
export const API_URL = `https://utask.kstu.kg/educations/api/v1/edu/`
...
$api_edu.interceptors.request.use((config) => {
  config.headers.Authorization = `Bearer ${auth_data.access}`
  return config
})
$api_edu.interceptors.response.use(
  (response) => response,
  async (error) => {
    if (error.response?.status === 401) {
      ...
      try {
        ...
        // BUG:исправить авторизацию сделать по другому
