  ...
  if (categoryKey === "test") {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 pt-3 sm:pt-4 w-full">
        {tests.length > 0 ? (
          tests.map((item) => <TestCard key={item.id} item={item} course_id={data.id as string}/>)
        ) : ( 
      ...
      {themes.map((theme) => {
        ...
        return (
          <Card
            ...
            className={`transition-all duration-300 justify-center py-2 ${isThemeExpanded ? "col-span-1 sm:col-span-2 lg:col-span-3" : ""
              }`}
          >
            <CardContent className="flex flex-col justify-between p-3 sm:p-4">
              <div
                className={`flex items-center mb-2 gap-2 ${!isStudent ? "justify-between" : "justify-between flex-wrap"
                  }`}
              >
              ...
              <div className="flex items-start justify-between gap-2 pb-2">
                <span
                  className={`text-base sm:text-lg font-semibold flex-1 ${theme.locked ? "blur-sm pointer-events-none select-none" : ""
                    }`}
                >
                  {theme.title}
                </span>
              ...
              <span
                className={`text-sm sm:text-base text-foreground/80 mb-2 sm:mb-3 line-clamp-2 sm:line-clamp-none ${theme.locked ? "blur-sm pointer-events-none select-none" : ""
                  }`}
              >
                {theme.description}
              </span>
...
export const ListThemes: React.FC<ThemeViewProps> = ({
  ...
  if (categoryKey === "test") {
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 pt-3 sm:pt-4">
        {tests?.map((item) => (
          <TestCard key={item.id} item={item} course_id={data.id as string} />
        ))}
      </div>
    );
  }
  ...
  return (
    ...
    >
      {themes.map((theme) => (
        ...
        >
          <AccordionTrigger
            className={`px-3 sm:px-5 py-3 sm:py-4 hover:no-underline items-center touch-manipulation ${theme.locked ? "cursor-not-allowed opacity-60" : "cursor-pointer"
              }`}
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full gap-2 sm:gap-4">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 md:gap-4 flex-1 min-w-0">
                <span
                  className={`text-base sm:text-lg font-semibold ${theme.locked ? "blur-sm pointer-events-none select-none" : ""
                    }`}
                >
                  {theme.title}
                </span>
                {!isMobile && (
                  <span
                    className={`text-xs text-foreground/80 hidden md:inline ${theme.locked
          ...
          <AccordionContent className="px-3 sm:px-5 py-3 sm:py-4">
            <div className="flex flex-col gap-3 sm:gap-4">
              {isMobile && (
                <span
                  className={`text-sm text-foreground/80 ${theme.locked ? "blur-sm pointer-events-none select-none" : ""
                    }`}
                >
                  {theme.description}
                </span>
