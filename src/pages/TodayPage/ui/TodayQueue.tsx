...
const STUDENT_SECTIONS: { id: StudentBucket; title: string }[] = [
  { id: "overdue", title: "Просрочено" },
  { id: "open", title: "Сделать сейчас" },
  { id: "upcoming", title: "Откроется в ближайшие 7 дней" },
  { id: "waiting", title: "На проверке" },
];
...
function studentWhen(theme: TodayTheme, bucket: StudentBucket) {
  if (bucket === "upcoming" && theme.opening) {
    return `откроется ${formatDayLabel(theme.opening)}`;
  }
  if (bucket === "waiting" && theme.submittedAt) {
    return `сдано ${format(theme.submittedAt, "d MMM, HH:mm", { locale: ru })}`;
  }
  if (bucket === "overdue" && theme.deadline) {
    return `срок был ${formatDayLabel(theme.deadline)}`;
  }
  if (theme.deadline) return `до ${formatDayLabel(theme.deadline)}`;
  return "";
}
...
export function TodayQueue({
    ...
    return (
      <section className="flex flex-col gap-2">
        <div className="flex items-baseline justify-between gap-3">
          <h2 className="text-sm font-semibold">На проверку</h2>
          <span className="text-xs text-muted-foreground">
            {rows.length === 0 ? "пусто" : `${rows.length} работ`}
          </span>
        </div>
        {rows.length === 0 ? (
  ...
  const grouped = STUDENT_SECTIONS.map((section) => ({
    ...section,
    items: themes
  ...
  if (!grouped.length) {
    return (
      <p className="rounded-2xl border border-dashed px-4 py-8 text-center text-sm text-muted-foreground">
        {courseId
          ? "По этому курсу сейчас нечего сдавать"
          : "Сейчас нечего сдавать"}
      </p>
    );
  }
